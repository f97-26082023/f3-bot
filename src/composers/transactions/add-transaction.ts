...
export async function addTransaction(ctx: MyContext) {
  ...
  try {
    ...
    const validInput = /^(?<amountOnly>\d{1,}(?:[.,]\d+)?([-+/*^]\d{1,}(?:[.,]\d+)?)*[k|M|m|G|g|T|t|P|p|E|e]?[1-9]?)*$|(?<description>.+)\s(?<amount>\d{1,}(?:[.,]\d+)?([-+/*^]\d{1,}(?:[.,]\d+)?)*[k|M|m|G|g|T|t|P|p|E|e]?[1-9]?)$/
...
async function newTransactionCategoryCbQH(ctx: MyContext) {
  ...
  try {
    ...
    return ctx.editMessageText(
      ctx.i18n.t('transactions.add.transactionError', { message: (err as any).message })
    )
  }
}
...
async function deleteTransactionActionHandler(ctx: MyContext) {
  ...
  try {
    ...
    if (trId || !isNaN(trId)) await firefly(userId).Transactions.deleteTransaction(trId as any)
