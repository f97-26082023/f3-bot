...
async function doDeleteCategoryCbQH(ctx: MyContext) {
  ...
  try {
    ...
    await firefly(userId).Categories.deleteCategory(categoryId as any)
    ...
    log('Error: %O', err)
    console.error(err)
    ctx.reply('Error occurred deleting a category: ', (err as any).message)
  }
}
...
function addCategoriesRouteHandler(ctx: MyContext) {
  ...
  try {
    ...
    return ctx.reply(ctx.i18n.t('categories.confirmNewCategory', {
...
async function showCategoryDetails(ctx: MyContext) {
  ...
  try {
    ...
    const start = moment(startDate).startOf('month') as any
    const end = moment(startDate).endOf('month') as any
    log('start: %O', start)
    log('end: %O', end)

    const categoryPromise = firefly(userId).Categories.getCategory(categoryId as any)
    const categoryTransactionsPromise = firefly(userId).Categories
      .listTransactionByCategory(categoryId as any, 1, start, end)
    ...
    const text = ctx.i18n.t('categories.transactionsList', {
      ...
      monthName: getMonthNameCapitalized(moment(startDate)),
...
function createSingleCategoryKeyboard(
  ...
  const thisMonthName = getMonthNameCapitalized(moment(curMonth))
...
function getMonthNameCapitalized(date: Moment) {
  return moment(date).format('MMMM, YYYY').replace(/^./, c => c.toLocaleUpperCase())
}
